...
def _sample_from_joint(pj: np.ndarray) -> Tuple[float, float]:
    """Draw a correlated (phi_rad, psi_rad) pair with continuous uniform jitter within the bin."""
    p = np.array(pj, dtype=np.float64).ravel()
    p = np.clip(p, 0, None);  p /= p.sum()
    k = np.random.choice(N_BINS * N_BINS, p=p)
    
    # Base bin centers
    phi_base = -180.0 + (k // N_BINS + 0.5) * BIN_WIDTH
    psi_base = -180.0 + (k %  N_BINS + 0.5) * BIN_WIDTH
    
    # Add continuous uniform noise WITHIN the chosen bin (+/- 5 degrees)
    phi_deg = phi_base + np.random.uniform(-BIN_WIDTH / 2, BIN_WIDTH / 2)
    psi_deg = psi_base + np.random.uniform(-BIN_WIDTH / 2, BIN_WIDTH / 2)
...
def sample_langevin_torsions(
    ...
    N_closure:        np.ndarray,
    probs_joint:      list,
    n_steps:          int   = 200,     # Faster because KIC already did the hard work
    base_lr:          float = 0.02,
    base_noise:       float = 0.05,    # Low noise: jiggle out of clashes without breaking closure
    closure_weight:   float = 100.0,
    framework_grid:   Optional[FrameworkGrid] = None,
    loop_radii:       Optional[np.ndarray] = None,
    k_clash:          float = 100.0,
    clash_weight:     float = 50.0, 
    clash_cap:        float = DEFAULT_CLASH_CAP,
    clash_buffer:     float = DEFAULT_CLASH_BUFFER,
    boundary_coords:  Optional[np.ndarray] = None,
    boundary_radii:   Optional[np.ndarray] = None,
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Adam-Preconditioned Langevin Dynamics (initialized from KIC).
    """
    import math
    B, n_loop = phi_init.shape
    
    # 1. Initialize exactly from KIC (split into leaves for Adam)
    phi = phi_init.clone().requires_grad_(True)
    psi_anc = psi_init[:, 0:1].clone().requires_grad_(True)
    psi_body = psi_init[:, 1:-1].clone().requires_grad_(True)
    psi_clos = psi_init[:, -1:].clone().requires_grad_(True)

    N_close_t = torch.tensor(N_closure, dtype=torch.float32).unsqueeze(0)
    
    optimizer = torch.optim.Adam([
        {'params': [phi, psi_body], 'lr': base_lr},
        {'params': [psi_anc, psi_clos], 'lr': base_lr * 2.0} 
    ])

    intra_idx_i, intra_idx_j = _build_intraloop_exclusion_mask(n_loop)
    intra_radii = torch.tensor([VDW_RADII['N'], VDW_RADII['CA'], VDW_RADII['C']] * n_loop, dtype=torch.float32)
    loop_radii_t = torch.tensor(loop_radii, dtype=torch.float32) if loop_radii is not None else None
    
    bnd_coords_t, bnd_radii_t = None, None
    if boundary_coords is not None:
        bnd_coords_t = torch.tensor(boundary_coords, dtype=torch.float32)
        bnd_radii_t  = torch.tensor(boundary_radii,  dtype=torch.float32)
        bnd_idx_loop, bnd_idx_bnd = _build_boundary_exclusion_pairs(n_loop, len(boundary_radii))

    best_score = torch.full((B,), float('inf'))
    best_phi   = phi.detach().clone()
    best_psi   = torch.cat([psi_anc, psi_body, psi_clos], dim=1).detach().clone()
    ...
    for step in range(n_steps):
        ...
        # Ramp clash quadratically so it gently nudges before getting strict
        w_closure = closure_weight 
        w_clash   = clash_weight * (progress ** 2) 

        psi = torch.cat([psi_anc, psi_body, psi_clos], dim=1)
        
        N, CA, C = build_backbone(phi, psi, anchor_N, anchor_CA, anchor_C)
        N_virt   = place_N_after(N[:, -1], CA[:, -1], C[:, -1], psi[:, -1])
        
        e_neural  = compute_energy(phi, psi, probs_joint)
        e_closure = torch.sum((N_virt - N_close_t) ** 2, dim=-1)
        e_intra = torch.tensor(0.0); e_fw = torch.tensor(0.0); e_bnd = torch.tensor(0.0)
        
        if len(intra_idx_i) > 0:
            e_intra = compute_intraloop_clash_energy(N, CA, C, intra_idx_i, intra_idx_j, intra_radii, k_clash=k_clash, cap=clash_cap, buffer=clash_buffer)
        if framework_grid is not None and loop_radii_t is not None:
            loop_atoms = torch.stack([N, CA, C], dim=2).reshape(B, -1, 3)
            e_fw = framework_grid.query_energy(loop_atoms, loop_radii_t, k_clash=k_clash, cap=clash_cap)
        if bnd_coords_t is not None:
            e_bnd = compute_boundary_clash_energy(N, CA, C, bnd_coords_t, bnd_radii_t, intra_radii, bnd_idx_loop, bnd_idx_bnd, k_clash=k_clash, cap=clash_cap, buffer=clash_buffer)

        total_energy = e_neural + w_closure * e_closure + w_clash * (e_intra + e_fw + e_bnd)
        loss = total_energy.sum()
        loss.backward()

        torch.nn.utils.clip_grad_value_([phi, psi_anc, psi_body, psi_clos], clip_value=5.0)
        optimizer.step()

        # --- LANGEVIN NOISE INJECTION ---
        with torch.no_grad():
            noise_scale = base_noise * (1.0 - progress) 
            
            for p in [phi, psi_anc, psi_body, psi_clos]:
                p.add_(torch.randn_like(p) * noise_scale)
                p.copy_(torch.remainder(p + math.pi, 2 * math.pi) - math.pi)
            
            psi_full = torch.cat([psi_anc, psi_body, psi_clos], dim=1)
            
            # Save the best physically valid states
            cl_dist = torch.sqrt(e_closure)
            clash_sum = e_intra + e_fw + e_bnd
            combined_score = clash_sum + e_neural
            
            # Only save states where closure is preserved (< 0.5A)
            valid = cl_dist < 0.5
            improved = (combined_score < best_score) & valid
            
            best_score[improved] = combined_score[improved]
            best_phi[improved] = phi.detach()[improved]
            best_psi[improved] = psi_full.detach()[improved]

        if step % 50 == 0 or step == n_steps - 1:
            total_clash = e_intra.mean().item() + e_fw.mean().item() + e_bnd.mean().item()
            print(f"        Step {step:3d} | "
                  f"Energy: {e_neural.mean().item():.2f} | "
                  f"Closure: {cl_dist.mean().item():.3f}Å | "
                  f"Clash: {total_clash:.2f} ")

    # Fallback for any that never hit valid closure
    never = best_score == float('inf')
    if never.any():
        best_phi[never] = phi.detach()[never]
        best_psi[never] = torch.cat([psi_anc, psi_body, psi_clos], dim=1).detach()[never]
