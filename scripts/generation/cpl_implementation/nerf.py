...
# Add this function to nerf.py after build_loop()
...
def compute_O_atoms(
    ...
    For interior residues, O is placed by bisecting the CA-C and C-N(next)
    vectors.  For the final residue the CA-C vector is used alone.
    Bond length C=O = 1.229 Angstroms.
    """
    n = len(CA)
    O = np.zeros((n, 3))
    for i in range(n):
        v_ca = CA[i] - C[i]
        v_ca /= np.linalg.norm(v_ca) + 1e-8
        if i < n - 1:
            v_n = N[i+1] - C[i]
            v_n /= np.linalg.norm(v_n) + 1e-8
            bis = v_ca + v_n
