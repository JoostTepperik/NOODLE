...
def extract_framework_atoms(
    ...
    Includes all chains in the complex except:
      - Loop residues on tcr_chain  (loop_start:loop_end into full_sequence)
      - Flank residues on tcr_chain (the NeRF N- and C-terminal anchors)

    Atoms per residue: N, CA, C, O, CB.  CB is skipped for glycine.

    Args:
        complex_pdb:    path to full complex PDB
        tcr_chain:      chain containing the CDR3 loop (e.g. 'E')
        full_sequence:  loop + flank context string used to locate the loop
                        within tcr_chain
        loop_start:     loop start index into full_sequence
        loop_end:       loop end index into full_sequence
        n_flank_before: N-terminal flank residue count (excluded as anchors)
        n_flank_after:  C-terminal flank residue count (excluded as anchors)

    Returns:
        coords: (N_atoms, 3)  float32 — framework atom Cartesian positions
        radii:  (N_atoms,)    float32 — per-atom vdW radii
    """
    # Locate loop + flank residue numbers in the TCR chain
    tcr_resnums = _parse_chain_resnums(complex_pdb, tcr_chain, full_sequence)
    if tcr_resnums is None:
        warnings.warn(
            f"extract_framework_atoms: could not locate full_sequence in "
            f"chain {tcr_chain} of {complex_pdb} — no exclusion applied"
        )
        exclude_resnums: set = set()
    else:
        excl_idx = (set(range(loop_start, loop_end))          # loop
                    | set(range(0, n_flank_before))            # N-flank
                    | set(range(loop_end, loop_end + n_flank_after)))  # C-flank
    ...
    print(f"    Framework: {len(coords):,} atoms  ({complex_pdb})")
    return coords, radii
