...
# ─────────────────────────────────────────────────────────────────────────────
# Unconstrained XYZ baseline
#
# Free variables: raw N, CA, C positions for every loop residue.
# Everything is soft — all geometry enforced by loss terms only.
# Meant as a negative control to show that exact geometry constraints matter.
#
# Loss terms:
#   bond_weight   * (L1 on all 3 bond lengths per residue + all C→N junctions)
#   angle_weight  * (L2 on all 5 angles per junction: N-CA-C, CA-C-N, C-N-CA,
#                    + omega planarity, + CA-C-N-CA improper)
#   energy        * Ramachandran -log p
#   closure_weight * progress * (closure distance - BL_CN)²
# ─────────────────────────────────────────────────────────────────────────────

def _angle_loss(a, b, c, target_rad):
    """L2 on the angle at b between vectors b→a and b→c. (B,3) inputs → scalar."""
    v1 = a - b;  v1 = v1 / (v1.norm(dim=-1, keepdim=True) + 1e-8)
    v2 = c - b;  v2 = v2 / (v2.norm(dim=-1, keepdim=True) + 1e-8)
    cos_a = (v1 * v2).sum(-1).clamp(-1 + 1e-6, 1 - 1e-6)
    angle = torch.acos(cos_a)
    return ((angle - target_rad) ** 2).mean()


def _omega_loss(CA_prev, C_prev, N_cur, CA_cur):
    """Penalise deviation from trans (|omega| = 180°). (B,3) inputs → scalar."""
    omega = _dihedral_torch(CA_prev, C_prev, N_cur, CA_cur)
    # cos(omega) should be -1 for trans
    return (torch.cos(omega) + 1.0).pow(2).mean()


def optimize_xyz_unconstrained(
    N_init:    torch.Tensor,   # (B, n, 3)  random initial positions
    CA_init:   torch.Tensor,
    C_init:    torch.Tensor,
    anchor_N:  np.ndarray,     # (3,) last flank_before N
    anchor_CA: np.ndarray,     # (3,) last flank_before CA
    anchor_C:  np.ndarray,     # (3,) last flank_before C
    N_closure: np.ndarray,     # (3,) first flank_after N
    probs_phi: list,
    probs_psi: list,
    n_steps:        int   = 1000,
    lr:             float = 0.05,
    ...
    N  = N_init.clone().requires_grad_(True)
    CA = CA_init.clone().requires_grad_(True)
    C  = C_init.clone().requires_grad_(True)

    anc_N_t  = torch.tensor(anchor_N,  dtype=torch.float32).unsqueeze(0).expand(B, -1)
    anc_CA_t = torch.tensor(anchor_CA, dtype=torch.float32).unsqueeze(0).expand(B, -1)
    anc_C_t  = torch.tensor(anchor_C,  dtype=torch.float32).unsqueeze(0).expand(B, -1)
    clos_N_t = torch.tensor(N_closure, dtype=torch.float32).unsqueeze(0).expand(B, -1)

    optimizer = torch.optim.Adam([N, CA, C], lr=lr)

    best_score = torch.full((B,), float('inf'))
    best_N  = N_init.clone()
    best_CA = CA_init.clone()
    best_C  = C_init.clone()

    print(f"      XYZ unconstrained  B={B}  n_loop={n}  n_steps={n_steps}")

    for step in range(n_steps):
        optimizer.zero_grad()

        # ── Bond lengths ──────────────────────────────────────────────────────
        l_bond = torch.tensor(0.0)
        # Intra-residue
        l_bond = l_bond + ((N  - CA).norm(dim=-1) - BL_NCA).abs().mean()
        l_bond = l_bond + ((CA - C ).norm(dim=-1) - BL_CAC).abs().mean()
        # Anchor → N[0]
        l_bond = l_bond + ((N[:, 0] - anc_C_t).norm(dim=-1) - BL_CN).abs().mean()
        # Inter-residue C→N
        if n > 1:
            l_bond = l_bond + ((N[:, 1:] - C[:, :-1]).norm(dim=-1) - BL_CN).abs().mean()

        # ── Angles ────────────────────────────────────────────────────────────
        l_angle = torch.tensor(0.0)
        # Intra-triangle N-CA-C
        l_angle = l_angle + _angle_loss(N, CA, C,
                                        torch.tensor(BA_NCA_C))
        # Inter-triangle angles at each junction
        for i in range(n):
            C_prev  = anc_C_t        if i == 0 else C[:, i-1]
            CA_prev = anc_CA_t       if i == 0 else CA[:, i-1]
            N_next  = clos_N_t       if i == n-1 else N[:, i+1]
            # CA-C-N (angle at C[i])
            l_angle = l_angle + _angle_loss(CA[:, i], C[:, i], N_next,
                                            torch.tensor(BA_CA_CN))
            # C-N-CA (angle at N[i])
            l_angle = l_angle + _angle_loss(C_prev, N[:, i], CA[:, i],
                                            torch.tensor(BA_CN_CA))
            # omega: CA[i-1]-C[i-1]-N[i]-CA[i] should be 180°
            l_angle = l_angle + _omega_loss(CA_prev, C_prev, N[:, i], CA[:, i])
        ...
        # ── Closure ───────────────────────────────────────────────────────────
        cl_dist  = (C[:, -1] - clos_N_t).norm(dim=-1)
        progress = step / max(n_steps - 1, 1)
        l_closure = ((cl_dist - BL_CN) ** 2).mean()

        loss = (energy
                + bond_weight  * l_bond
                + angle_weight * l_angle
                + closure_weight * progress * l_closure)
        loss.backward()
        optimizer.step()
        ...
        # Checkpoint
        with torch.no_grad():
            score = energy.detach() + closure_weight * cl_dist
            improved = score < best_score
            best_score[improved] = score[improved]
            best_N[improved]  = N.detach()[improved]
            best_CA[improved] = CA.detach()[improved]
            best_C[improved]  = C.detach()[improved]

        if step % 200 == 0 or step == n_steps - 1:
            with torch.no_grad():
                bl_nca = (N - CA).norm(dim=-1).mean().item()
                bl_cac = (CA - C).norm(dim=-1).mean().item()
                ang    = torch.rad2deg(torch.acos(
                    ((N-CA)/((N-CA).norm(dim=-1,keepdim=True)+1e-8) *
                     (C-CA)/((C-CA).norm(dim=-1,keepdim=True)+1e-8)).sum(-1).clamp(-1,1)
                )).mean().item()
            print(f"        step {step:4d}:  E={energy.item():.2f}  "
                  f"bond={l_bond.item():.4f}  angle={l_angle.item():.4f}  "
                  f"N-CA={bl_nca:.3f}Å  CA-C={bl_cac:.3f}Å  "
                  f"N-CA-C={ang:.1f}°  closure={cl_dist.mean().item():.4f}Å")

    return best_N, best_CA, best_C


def refine_loop_xyz_unconstrained(
    full_sequence,
    loop_start, loop_end,
    N_flank_before, CA_flank_before, C_flank_before, O_flank_before,
    N_flank_after,  CA_flank_after,  C_flank_after,  O_flank_after,
    ...
    bond_weight=100.0, angle_weight=50.0, closure_weight=50.0,
    n_structures=10, position_scale=10.0, seed=None,
):
    """
    Unconstrained XYZ baseline: optimise raw atom positions with soft geometry.
    Returns ensemble in the same format as refine_loop_se3_fixed for direct
    comparison.
    """
    from loop_modeling_nerf import cache_energy_distributions
    from random_backbone_cloud import generate_random_backbone_cloud
    ...
    print(f"\n  XYZ-unconstrained baseline:")
    print(f"    Sequence: {loop_seq} ({n_loop} res)")
    ...
    for idx in range(n_structures):
        ...
        N_c, CA_c, C_c, _ = generate_random_backbone_cloud(
            n_residues=n_loop, position_scale=position_scale, seed=s)
        all_N.append(torch.tensor(N_c,  dtype=torch.float32))
        all_CA.append(torch.tensor(CA_c, dtype=torch.float32))
        all_C.append(torch.tensor(C_c,  dtype=torch.float32))

    N_batch  = torch.stack(all_N)
    CA_batch = torch.stack(all_CA)
    C_batch  = torch.stack(all_C)

    N_opt, CA_opt, C_opt = optimize_xyz_unconstrained(
        N_batch, CA_batch, C_batch,
    ...
    with torch.no_grad():
        phi_pos, psi_pos = extract_phi_psi_from_positions(
            N_opt, CA_opt, C_opt, anc_C_t, clos_N_t)
        cl_all = (C_opt[:, -1] - clos_N_t).norm(dim=-1)
        e_all  = compute_energy(phi_pos, psi_pos, probs_phi, probs_psi)

    print(f"\n    Results ({n_structures} structures):")
    for idx in range(n_structures):
        print(f"      [{idx+1:2d}] closure={cl_all[idx].item():.4f}Å  E={e_all[idx].item():.2f}")

    print(f"\n    After optimization (structure 1):")
    validate_geometry(N_opt[0].numpy(), CA_opt[0].numpy(), C_opt[0].numpy(),
                      label="xyz_unconstrained")

    ensemble = []
    for idx in range(n_structures):
        N_np  = N_opt[idx].numpy()
        CA_np = CA_opt[idx].numpy()
        C_np  = C_opt[idx].numpy()
        O_np  = compute_O_atoms(N_np, CA_np, C_np)
        ensemble.append((
            np.vstack([N_flank_before,  N_np,  N_flank_after]),
            np.vstack([CA_flank_before, CA_np, CA_flank_after]),
